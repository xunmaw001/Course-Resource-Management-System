    ...
    <script>
        ...
        function setDataRow(item, number) {
            ...
            row.appendChild(kechengRenzhiyaoqiuCell);
                ...
                //课程详情
            var kechengContentCell = document.createElement('td');
            //如果图文详情包含图片
            if (item.kechengContent && item.kechengContent.indexOf('img') != -1) {
                //暂时只考虑图片+文字 和 文字+图片的情况
                var beginIndex = item.kechengContent.indexOf('<img');
                var endIndex = item.kechengContent.indexOf('>');
                    kechengContentCell.innerHTML = item.kechengContent.substring(beginIndex, endIndex + 1).replace("img", "img width='100' height='100'");
            } else {
                if (item.kechengContent != null && item.kechengContent != "" && item.kechengContent.length >= 11) {
                        kechengContentCell.innerHTML = item.kechengContent.substring(0, 10) + "...";
                } else {
                        kechengContentCell.innerHTML = item.kechengContent;
                }
            }
            row.appendChild(kechengContentCell);
